...
@section('scripts')
    ...
    @include('examen-cab.modals.crear-examen-cab')
    </script>
    <script src="{{URL::asset('public/plugins/bootbox/bootbox.min.js')}}"></script>
    <script>

        var url_index ="{{route('examen-cab.index')}}";
        ...
        $(document).on('click','.btn-actualizar-estado',function () {
            ...
            var dialog = bootbox.dialog({
                title: "<b>¡ALERTA!</b>",
                message: "<b>¿Estás seguro de "+accion+" el tipo de examen?</b>",
                size: 'medium',
                buttons: {
                    cancel: {
                        label: "CANCELAR",
